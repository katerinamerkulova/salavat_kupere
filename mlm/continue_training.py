...
model = SentenceTransformer(model_name, device=2)

# Convert the dataset to a DataLoader ready for training
logging.info("Read STSbenchmark train dataset")

dataset = pd.read_parquet("data/train_pairs.parquet")
etl = pd.read_parquet("data/train_data_minilm.parquet")

samples = []
for i, row in dataset.iterrows():
    text1 = etl.loc[etl['variantid'] == row['variantid1'], 'name'].item()
    text2 = etl.loc[etl['variantid'] == row['variantid2'], 'name'].item()
